package yaydev.notes.app.view.NemosoftsText;
...
public class NemosoftsEditText extends EditText implements TextWatcher {
    ...
    public NemosoftsEditText(Context context) {
        super(context);
        init(null);
    }

    public NemosoftsEditText(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs);
    }

    public NemosoftsEditText(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(attrs);
    }

    @SuppressWarnings("NewApi")
    public NemosoftsEditText(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        super(context, attrs, defStyleAttr, defStyleRes);
        init(attrs);
    }

    private void init(AttributeSet attrs) {
        TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.NemosoftsTextView);
        bulletColor = array.getColor(R.styleable.NemosoftsTextView_bulletColor, 0);
        bulletRadius = array.getDimensionPixelSize(R.styleable.NemosoftsTextView_bulletRadius, 0);
        bulletGapWidth = array.getDimensionPixelSize(R.styleable.NemosoftsTextView_bulletGapWidth, 0);
        historyEnable = array.getBoolean(R.styleable.NemosoftsTextView_historyEnable, true);
        historySize = array.getInt(R.styleable.NemosoftsTextView_historySize, 100);
        linkColor = array.getColor(R.styleable.NemosoftsTextView_linkColor, 0);
        linkUnderline = array.getBoolean(R.styleable.NemosoftsTextView_linkUnderline, true);
        quoteColor = array.getColor(R.styleable.NemosoftsTextView_quoteColor, 0);
        quoteStripeWidth = array.getDimensionPixelSize(R.styleable.NemosoftsTextView_quoteStripeWidth, 0);
        quoteGapWidth = array.getDimensionPixelSize(R.styleable.NemosoftsTextView_quoteCapWidth, 0);
        array.recycle();
    ...
    protected void styleInvalid(int style, int start, int end) {
        ...
        List<NemosoftsPart> list = new ArrayList<>();

        for (StyleSpan span : spans) {
            if (span.getStyle() == style) {
                list.add(new NemosoftsPart(getEditableText().getSpanStart(span), getEditableText().getSpanEnd(span)));
                getEditableText().removeSpan(span);
            }
        }

        for (NemosoftsPart part : list) {
            if (part.isValid()) {
    ...
    protected void bulletValid() {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (bulletStart < bulletEnd) {
                getEditableText().setSpan(new NemosoftsBulletSpan(bulletColor, bulletRadius, bulletGapWidth), bulletStart, bulletEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
        }
    }
    ...
    protected void quoteValid() {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (quoteStart < quoteEnd) {
                getEditableText().setSpan(new NemosoftsQuoteSpan(quoteColor, quoteStripeWidth, quoteGapWidth), quoteStart, quoteEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
        }
    }
    ...
    protected void linkValid(String link, int start, int end) {
        ...
        linkInvalid(start, end);
        getEditableText().setSpan(new NemosoftsURLSpan(link, linkColor, linkUnderline), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
    }
    ...
    public void fromHtml(String source) {
        ...
        builder.append(NemosoftsParser.fromHtml(source));
        switchToKnifeStyle(builder, 0, builder.length());
        setText(builder);
    }

    public String toHtml() {
        return NemosoftsParser.toHtml(getEditableText());
    }
    ...
    protected void switchToKnifeStyle(Editable editable, int start, int end) {
        ...
        for (BulletSpan span : bulletSpans) {
            ...
            editable.setSpan(new NemosoftsBulletSpan(bulletColor, bulletRadius, bulletGapWidth), spanStart, spanEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        ...
        for (QuoteSpan span : quoteSpans) {
            ...
            spanEnd = 0 < spanEnd && spanEnd < editable.length() && editable.charAt(spanEnd) == '\n' ? spanEnd - 1 : spanEnd;
            editable.removeSpan(span);
            editable.setSpan(new NemosoftsQuoteSpan(quoteColor, quoteStripeWidth, quoteGapWidth), spanStart, spanEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        ...
        for (URLSpan span : urlSpans) {
            ...
            int spanEnd = editable.getSpanEnd(span);
            editable.removeSpan(span);
            editable.setSpan(new NemosoftsURLSpan(span.getURL(), linkColor, linkUnderline), spanStart, spanEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
    }
}
